...
"""
    ...
    sentence encoder for u: StructuredSelfAttentive (multi, matrix);
...
### SSA sent encoder for context (matrix)
...
con_sent_att_mat = StructuredSelfAttentive(da=da, r=r, return_matrix=True)(con_sent_wa)
con_sent_att_mat_dr = Dropout(drop_rate)(con_sent_att_mat)
print ('single sentence encoder out', con_sent_att_mat_dr.shape)
con_sent_encoder = Model(con_sent_ints, con_sent_att_mat_dr)

### context encoder with SSA sent encoders
con_ints = Input(shape=(docs_train.shape[2]-1, docs_train.shape[3],))
print ('context encoder TimeDistributed in', con_ints.shape)
con_sent_att_mats_dr = TimeDistributed(con_sent_encoder, input_shape=(6, 10))(con_ints)
print ('context encoder TimeDistributed out', con_sent_att_mats_dr.shape)
context_encoder = Model(con_ints, con_sent_att_mats_dr)
...
print ('context encoder input', context_ints.shape)
context_mats = context_encoder(context_ints)
print ('context encoder out', context_mats.shape)
sent_att_vec = SentMultiContextAttention()([sent_wa, context_mats])
