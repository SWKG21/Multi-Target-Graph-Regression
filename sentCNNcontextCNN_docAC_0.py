...
"""
    ...
    sentence encoder for u: CNN; CNN as combination;
...
x = Lambda(lambda x: K.expand_dims(x, axis=1))(con_sent_wv_dr)  # (batch_size, 1, sent_len, embedding_dim)
convs = []
for window in cnn_windows:
    ...
    convs.append(tmp)
con_sent_att_vec = Concatenate(axis=1)(convs)  # (batch_size, #filters*#windows)
con_sent_att_vec_dr = Dropout(drop_rate)(con_sent_att_vec)
con_sent_encoder = Model(con_sent_ints, con_sent_att_vec_dr)

### context encoder with CNN sent encoders
con_ints = Input(shape=(docs_train.shape[2]-1, docs_train.shape[3],))
con_sent_att_vecs_dr = TimeDistributed(con_sent_encoder)(con_ints)  # (batch_size, 6, #filters*#windows)
con_sent_att_vecs_dr = Lambda(lambda x: K.expand_dims(x, axis=1))(con_sent_att_vecs_dr)  # (batch_size, 1, 6, #filters*#windows)
convs = []
